--[[
by omar alsaray @AL_RAKN  \ @llJOKERll
اي استفسار او لديك مشكله تابع قناتي @llJOKERll
او مراسلتي ع الخاص
تم تعريب االترمنال من قبل زين العابدين
...
-- by omer alsaray TEle _ @AL_RAKN
...
local function run(msg, matches) -- by omer alsaray TEle _ @AL_RAKN
...
if tonumber(msg.from.id) ~= tonumber(sudo_id) then return "☔️هذا الاوامر للمطور الاساسي فقط 🌑" end
  ...
  for k, v in pairs( plugins_names( )) do  
      -- get the name 
      v = string.match (v, "(.*)%.lua") 
sendDocument(msg.to.id, "./plugins/"..v..".lua", msg.id, "💢┇ الملف مقدم من قناةجوكر ¦ֆ \n💢┇ تابع قناة السورس @llJOKERll")
...
sendDocument(msg.to.id, "./plugins/"..file..".lua", msg.id, "💢┇ الملف مقدم من قناة جوكر ¦ֆ \n💢┇ تابع قناة السورس @llJOKERll")
end
end
end
